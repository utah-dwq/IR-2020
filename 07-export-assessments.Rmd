...
```{r}
prelim_asmnts = subset(prelim_asmnts, !(prelim_asmnts$IR_MLID=="UTAHDWQ_WQX-5913210"&prelim_asmnts$R3172ParameterName=="Minimum Dissolved Oxygen"))
```
...
ecoli_assessed=as.data.frame(readxl::read_excel('99_external_assessments/ecoli/ecoli_assessed_review_12052019.xlsx', 'ecoli_mlid_asmnts'))
ecoli_adv_assessed=as.data.frame(readxl::read_excel('99_external_assessments/ecoli/3_draft_ecoliadvisory_assessment_v3_ef.xlsx', 'Sheet1'))
names(ecoli_adv_assessed)[names(ecoli_adv_assessed)=='AssessCat']='IR_Cat'
snowbird_assessed=as.data.frame(readxl::read_excel('99_external_assessments/snowbird/snowbird_asmnt.xlsx', 'asmnt'))
names(snowbird_assessed)[names(snowbird_assessed)=='AssessCat']='IR_Cat'
snowbird_assessed=within(snowbird_assessed, {
	IR_Lat=wqTools::facToNum(IR_Lat)
	IR_Long=wqTools::facToNum(IR_Long)
})
external_asmnts=plyr::rbind.fill(gsl_egg_se_assessed, fish_hg_assessed, hfdo_assessed, ecoli_assessed, ecoli_adv_assessed, snowbird_assessed)
```
...
```{r}
au_param_rej=as.data.frame(readxl::read_excel('08_secondary_reviews/COMPILED REJECTIONS/AU_param_rejections_SLCo_data_discrepancies_v2.xlsx', 'au-reviews'))
...
write.csv(file='08_secondary_reviews/site_use_param_asmnt_06242020.csv', site_use_param_asmnt, row.names=F)
```
...
lake_assessments = list(lake_tds_assessed,lake_tds_exc,lake_profs_assessed,prof_asmnts)
...
save(accepted_data, rejected_data, grouping_columns_rollup,lake_assessments, site_specific_tds_assessments, toxics_and_conventionals_assessments, prelim_asmnts, file='prelim_asmnts_06242020.Rdata')
```
...
save(assessed_data, criteria, file='asmntDashboard_data_withSLCO_v3.Rdata')
```
...
save(compiled_data, file = 'reviewer_export_data_withSLCO_v3.Rdata')
```
...
writexl::write_xlsx(list(
		...
		path = 'prelim_asmnts_withSLCO_v3-4Dec2019.xlsx', format_headers=F, col_names=T)
```
